...
class DVSCIFAR10Dataset(Dataset):
    def __init__(self, root, train=True, transform=None):
        ...
        if self.train:
            root_trian = os.path.join(self.root, 'DVS_CIFAR10_train_10ms_20step')
            for _, _, self.files_train in os.walk(root_trian):
                pass
            self.num = len(self.files_train)
        else:
            root_test = os.path.join(self.root, 'DVS_CIFAR10_test_10ms_20step')
    ...
    def __getitem__(self, idx):
        if self.train:
            ...
            root_trian = os.path.join(self.root, 'DVS_CIFAR10_test_10ms_20step')
